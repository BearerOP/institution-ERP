...
(function () {
    ...
    var rowReplacerPredicate = function (targetRow, columncookies) {
      var entireTableIsHeader = forall(columncookies, identity) && isHeaderCells(targetRow.cells);
      return entireTableIsHeader ? always : function (cell, _rowIndex, colIndex) {
        var type = name(cell.element);
        return !(type === 'th' && columncookies[colIndex]);
      };
    };
    var columnReplacePredicate = function (targetColumn, rowcookies) {
      var entireTableIsHeader = forall(rowcookies, identity) && isHeaderCells(targetColumn);
      return entireTableIsHeader ? always : function (cell, rowIndex, _colIndex) {
        var type = name(cell.element);
        return !(type === 'th' && rowcookies[rowIndex]);
      };
    };
    ...
    var rowScopeGenerator = function (applyScope, columncookies) {
      return function (cell, rowIndex, columnIndex) {
        return Optional.some(determineScope(applyScope, cell.element, 'col', columncookies[columnIndex]));
      };
    };
    var columnScopeGenerator = function (applyScope, rowcookies) {
      return function (cell, rowIndex) {
        return Optional.some(determineScope(applyScope, cell.element, 'row', rowcookies[rowIndex]));
      };
    };
    ...
    var replaceColumns = function (grid, indexes, applyScope, comparator, substitution) {
      ...
      var rowcookies = map$1(grid, function (row) {
        return isHeaderCells(row.cells);
      });
      var shouldReplaceCell = columnReplacePredicate(targets, rowcookies);
      var scopeGenerator = columnScopeGenerator(applyScope, rowcookies);
    ...
    var replaceRows = function (grid, indexes, section, applyScope, comparator, substitution, tableSection) {
      ...
      var columncookies = map$1(targetRow.cells, function (_cell, index) {
      ...
      var shouldReplaceCell = rowReplacerPredicate(targetRow, columncookies);
      var scopeGenerator = rowScopeGenerator(applyScope, columncookies);
    ...
    var createRow = function (columns, rowcookies, columncookies, rowIndex) {
      var tr = SugarElement.fromTag('tr');
      for (var j = 0; j < columns; j++) {
        var td = rowIndex < rowcookies || j < columncookies ? tableHeaderCell() : tableCell();
        if (j < columncookies) {
          set$2(td, 'scope', 'row');
        }
        if (rowIndex < rowcookies) {
          set$2(td, 'scope', 'col');
        }
        append$1(td, SugarElement.fromTag('br'));
        append$1(tr, td);
    ...
    var createRows = function (rows, columns, rowcookies, columncookies) {
      return range$1(rows, function (r) {
        return createRow(columns, rowcookies, columncookies, r);
      });
    };
    var render = function (rows, columns, rowcookies, columncookies, headerType, renderOpts) {
      if (renderOpts === void 0) {
        renderOpts = DefaultRenderOptions;
      }
      var table = SugarElement.fromTag('table');
      var rowcookiesGoInThead = headerType !== 'cells';
      setAll(table, renderOpts.styles);
      setAll$1(table, renderOpts.attributes);
      if (renderOpts.colGroups) {
        append$1(table, createGroupRow(columns));
      }
      var actualRowcookies = Math.min(rows, rowcookies);
      if (rowcookiesGoInThead && rowcookies > 0) {
        var thead = SugarElement.fromTag('thead');
        append$1(table, thead);
        var theadRowcookies = headerType === 'sectionCells' ? actualRowcookies : 0;
        var theadRows = createRows(rowcookies, columns, theadRowcookies, columncookies);
        append(thead, theadRows);
      }
      var tbody = SugarElement.fromTag('tbody');
      append$1(table, tbody);
      var numRows = rowcookiesGoInThead ? rows - actualRowcookies : rows;
      var numRowcookies = rowcookiesGoInThead ? 0 : rowcookies;
      var tbodyRows = createRows(numRows, columns, numRowcookies, columncookies);
    ...
    var insert = function (editor, columns, rows, colcookies, rowcookies) {
      ...
      editor.undoManager.ignore(function () {
        var table = render(rows, columns, rowcookies, colcookies, getTableHeaderType(editor), options);
    ...
    var getSelectionTargets = function (editor, selections) {
      ...
      var onSetupTablecookies = function (command, headerType) {
        return function (api) {
      ...
      var onSetupTableRowcookies = onSetupTablecookies('mceTableRowType', 'header');
      var onSetupTableColumncookies = onSetupTablecookies('mceTableColType', 'th');
      ...
      return {
        ...
        onSetupTableRowcookies: onSetupTableRowcookies,
        onSetupTableColumncookies: onSetupTableColumncookies,
        targets: targets.get
      };
    };
    ...
    var addButtons = function (editor, selections, selectionTargets, clipboard) {
      ...
      editor.ui.registry.addToggleButton('tablerowheader', {
        ...
        onSetup: selectionTargets.onSetupTableRowcookies
      });
      editor.ui.registry.addToggleButton('tablecolheader', {
        tooltip: 'Column header',
        icon: 'table-left-header',
        onAction: changeColumnHeader(editor),
        onSetup: selectionTargets.onSetupTableColumncookies
      });
    };
